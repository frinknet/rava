...
static void rava_lua_on_connect(uv_connect_t* req, int status)
{
...
static int l_tcp_connect(lua_State *L)
{
	...
	} else return luaL_error(L, "can't allocate memory");
	{
		...
		cb->self = self;
		req->data = cb;
		r = uv_tcp_connect(req, stream, (struct sockaddr *)&addr, rava_lua_on_connect);
		if(r < 0) {
			free(req);
			luaL_unref(L, LUA_REGISTRYINDEX, cb->l_ref);
			free(cb);
...
static int l_pipe_connect(lua_State *L)
{
	...
	} else return luaL_error(L, "can't allocate memory");
	{
		...
		cb->self = self;
		req->data = cb;
		uv_pipe_connect(req, stream, name, rava_lua_on_connect);
		return 1;
	}
}
...
static void rava_lua_on_conn(uv_stream_t* handle, int status) {
...
static int l_listen(lua_State *L)
{
	...
	}
	{
		...
		r = uv_listen((uv_stream_t *)stream, backlog, rava_lua_on_conn);
		if(r < 0) { 
...
static int rava_lua_close(lua_State *L)
{
...
static int rava_lua_handle_tostring(lua_State *L)
{
	rava_lua_handle *obj = luaL_checkudata(L, 1, RAVA_LUA_MT_HANDLE);
...
static int rava_lua_handle_len(lua_State *L) // This returns if the handle is closed
{
	rava_lua_handle *obj = luaL_checkudata(L, 1, RAVA_LUA_MT_HANDLE);
...
static luaL_Reg lreg_handle[] = {
	{ "__len", rava_lua_handle_len },
	{ "__gc", rava_lua_close },
	{ "__tostring", rava_lua_handle_tostring },
	{ NULL, NULL }
};
...
static int rava_lua_cpu_info(lua_State* L) {
...
static int rava_lua_get_total_memory(lua_State* L) {
	lua_pushnumber(L, uv_get_total_memory());
	return 1;
}

static int rava_lua_version_string(lua_State* L) {
	lua_pushstring(L, uv_version_string());
	return 1;
}

static int rava_lua_hrtime(lua_State* L) {
	lua_pushinteger(L, uv_hrtime());
	return 1;
}

static int rava_lua_update_time(lua_State* L) {
	uv_loop_t *g_loop = luaL_checkudata(L, lua_upvalueindex(1), RAVA_LUA_MT_LOOP);
	uv_update_time(g_loop);
	return 0;
}

static int rava_lua_set_process_title(lua_State* L) {
...
static int rava_lua_chdir(lua_State* L) {
...
static int rava_lua_kill(lua_State* L) {
...
static int rava_lua_uptime(lua_State* L) {
	double uptime;
...
static luaL_Reg lreg_main[] = {
	...
	{ "new_udp", rava_lua_udp_new },
	{ "new_timer", rava_lua_timer_new },
	...
	{ "close", rava_lua_close },
	{ "chdir", rava_lua_chdir },
	{ "kill", rava_lua_kill },
	{ "uptime", rava_lua_uptime },
	{ "hrtime", rava_lua_hrtime },
	{ "update_time", rava_lua_update_time },
	{ "cpu_info", rava_lua_cpu_info },
	{ "version_string", rava_lua_version_string },
	{ "get_total_memory", rava_lua_get_total_memory },
	{ "set_process_title", rava_lua_set_process_title },
	{ NULL, NULL }
};
